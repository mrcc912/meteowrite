...
exports.reportAPIuse = function(key, func_name)
{
    APIUser.findOne({key: key}, function(err, api){
	if(err) throw err;
	if(api)
	{
	    if(api.use_count[func_name])
		api.use_count[func_name] = key;
	    api.use_count[func_name] += key;
	}
	api.save(function(err, art){
	    if(err) throw err;
...
exports.userReadArticle = function(userID, articleID, dispFunc)
{
	...
	if(user)
	{
		...
		for(var i = 0; i<article.keywords.length; i++) 
		{
		    ...
		    if(article.readers.find(userID) < 0)
...
{
    Article.find({biline: author}, function(err, articles){
	if(err) throw err;
	if(articles)
	{
	    var keywords = [];
		...
		{
		    if(keywords.find(articles[index].keywords[word].text) < 0)
