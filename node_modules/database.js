...
exports.addKeywords = function(userID, keys)
{
...
exports.userReadArticle = function(userID, articleID)
{
    User.findOne({username: userID}, function(err, user){
	if(err) throw err;
	if(user)
	{
		...
		for(var key in article.keywords)
		{
		    user.keywords[key] += article.keywords[key];
		}
	    });
	}
    });
...
exports.getArticlesRelatedToFacebook = function(username, func)
{
    ...
    var recs = [];
    fql({token: token}).query('SELECT uid FROM user WHERE username = "mrcc912"', function(err, data) {
	if (err) {
            throw err;
	}
	var id = data[0].uid;
