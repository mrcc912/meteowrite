...
function createUser(id, api_key, params, func)
{
	...
	if(user)
	{
	    func("User with that ID already exists on our database");
	}
	else
	{
...
exports.getArticleReaderInterests = function(articleID, api_key, func)
{
	...
	if(article)
	{
	    ...
	    if(article.readers.length ==0)
		console.log("there are no readers for this article");
	    console.log("checking through this array of users:");
	    console.log(article.readers);
	    if(article.readers.length != 0)
	    User.find({id: {$in : article.readers}, api_key: api_key}, function(err2, readers){
		if(err2) throw err2;
		if(readers)
		{
		...
		{
		    func("This article has no readers");
		}
	    });
...
exports.userReadArticle = function(userID, articleID, api_key, func)
{
	...
	if(!user)
	{
	    ...
	    createUser(userID, api_key, params, function(newuser){
		var art_id = parseInt(articleID);
		var bool = 0;
		for(var i = 0; i < newuser.articlesRead.length; i = i + 1) {
		    if(newuser.articlesRead[i] == art_id) {
			bool = 1;
		    }
		}
		if(bool == 0) {
